...
@if ($menu->isNotEmpty())
    @if ($variant === 'desktop')
        <nav aria-label="{{ wp_get_nav_menu_name($name) ?: __('Primary Navigation', 'sage') }}" class="hidden lg:flex">
    ...
    @else
        <nav aria-label="{{ wp_get_nav_menu_name($name) ?: __('Primary Navigation', 'sage') }}">
